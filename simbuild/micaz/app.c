...
#line 227
    {
      ...
      if (sendAckC$AMPacket$destination(buf) == AM_BROADCAST_ADDR && TOS_NODE_ID % 2 == 1) {
          ...
          if (strcmp(msg->key, sendAckC$key_p[sim_node()]) == 0) {
              ...
              sim_log_debug(162U, "role", "Message before pairing received from %-14hhu|\n", sendAckC$pairing_address[sim_node()]);
              ...
              if (sendAckC$AMSend$send(sendAckC$pairing_address[sim_node()], &sendAckC$packet[sim_node()], sizeof(my_msg_t )) == SUCCESS) {
                  sim_log_debug(163U, "role", "pairing confirmation to node %hhu\n", sendAckC$pairing_address[sim_node()]);
                  sendAckC$locked[sim_node()] = TRUE;
                }
            }
          else {
              sim_log_debug(164U, "role", "keys do not match. msg received from %hhu\n", sendAckC$AMPacket$source(buf));
            }
        }
      else {
        if (sendAckC$AMPacket$destination(buf) == AM_BROADCAST_ADDR && TOS_NODE_ID % 2 == 0) {
            sim_log_debug(165U, "role", "!!!!!!!!!!!info key_c: %s\n", sendAckC$key_c[sim_node()]);
            if (strcmp(msg->key, sendAckC$key_c[sim_node()]) == 0) {

                sendAckC$pairing_address[sim_node()] = sendAckC$AMPacket$source(buf);
                sendAckC$paired[sim_node()] = TRUE;
                sim_log_debug(166U, "role", "Message before pairing received from %-14hhu|\n", sendAckC$pairing_address[sim_node()]);
              }
            else {
                sim_log_debug(167U, "role", "keys do not match. msg received from %hhu\n", sendAckC$AMPacket$source(buf));
              }
          }
        }
      if (sendAckC$AMPacket$source(buf) == sendAckC$pairing_address[sim_node()] && sendAckC$paired[sim_node()]) {
      ...
      if (__nesc_ntoh_uint16(msg->msg_type.nxdata) == 3) {
          ...
          sim_log_debug(168U, "role", "update last child location!\n");
        }
    }
...
#line 162
# 80 "/opt/tinyos-main-2.1.2/tos/interfaces/AMSend.nc"
inline static error_t /*AMQueueP.AMQueueImplP*/AMQueueImplP$0$AMSend$send(am_id_t arg_0xb73060a0, am_addr_t addr, message_t * msg, uint8_t len){
#line 80
  unsigned char __nesc_result;
#line 80

#line 80
  __nesc_result = TossimActiveMessageC$AMSend$send(arg_0xb73060a0, addr, msg, len);
#line 80
...
# 100 "/opt/tinyos-main-2.1.2/tos/interfaces/Send.nc"
inline static void /*AMQueueP.AMQueueImplP*/AMQueueImplP$0$Send$sendDone(uint8_t arg_0xb73076a8, message_t * msg, error_t error){
#line 100
  switch (arg_0xb73076a8) {
#line 100
    case 0U:
#line 100
...
#line 100
    default:
#line 100
      /*AMQueueP.AMQueueImplP*/AMQueueImplP$0$Send$default$sendDone(arg_0xb73076a8, msg, error);
#line 100
      break;
#line 100
    }
...
#line 290
{
  ...
  if (sendAckC$AMSend$send(sendAckC$pairing_address[sim_node()], &sendAckC$packet[sim_node()], sizeof(my_msg_t )) == SUCCESS && TOS_NODE_ID % 2 == 0) {
      sim_log_debug(169U, "radio_send", "radio_send: response message type: %d.\n", __nesc_ntoh_uint16(msg->msg_type.nxdata));
    }
}
...
#line 58
  ...
  if (sendAckC$AMSend$send(AM_BROADCAST_ADDR, &sendAckC$packet[sim_node()], sizeof(my_msg_t )) == SUCCESS && TOS_NODE_ID % 2 == 1) {
...
#line 185
{
  ...
  if (&sendAckC$packet[sim_node()] == buf && sendAckC$PacketAcknowledgements$wasAcked(buf)) {
      ...
      if (!sendAckC$paired[sim_node()]) {
          ...
          sim_log_debug(159U, "radio_ack", "Still in pairing phase\n");
        }
      else 
        {
          sendAckC$MilliTimer_pairing$stop();
